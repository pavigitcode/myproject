...
function territorial_division_creations($unique_id = "")
{

    $users = DB::table('territorial_division')->select("unique_id", "ter_division")
...
function hsn_code($unique_id = "")
{
    ...
    $hsn_code = DB::table('hsn_creation')->select("unique_id", "hsn_name", "hsn_code")->where(["is_active" => 1, "is_delete" => 0])->orderBy('id', 'ASC')->get();
...
// =============================mythili=============================================================
function designation_creations_officer($unique_id = "")
{

    $designation_creations = DB::table('designation_creation')->select("unique_id", "designation_name")->where(["is_active" => 1, "is_delete" => 0])->where('unique_id', '<>','641828393e9b989808')->where('unique_id','<>','641827f68916470107')->where('unique_id','<>','641827b4f11e514254')->where('unique_id','<>','6418277e41dbc55270')->where('unique_id','<>','6418280fc395690980')->orderBy('id', 'ASC')->get();
...
function designation_creations($unique_id = "")
{
    ...
    $designation_creations = DB::table('designation_creation')->select("unique_id", "designation_name")->where(["is_active" => 1, "is_delete" => 0])->where('unique_id', '<>','641aa47bd978820109')->where('unique_id','<>','641aa48c6613952540')->orderBy('id', 'ASC')->get();
...
// uiupdate start pavilap
function user_screen_check_up($section_unique_id = "")
{

    $screen_sections = DB::table('user_screen')
    ->select("unique_id", "user_screen_name","main_screen_unique_id","department_id", "actions")
    // ->groupBy('actions');
    // ->select("unique_id", "user_screen_name","main_screen_unique_id","department_id",
    // DB::raw('group_concat(actions) as actions'))
    ->where([["main_screen_unique_id", '=', $section_unique_id], ["is_delete", "=", 0]])->get();
    // ->where("main_screen_unique_id", '=', $section_unique_id)->get();

    // dd(sum($screen_sections));
    // ;->orderBy('id', 'ASC')
...
function user_permission_ui_update($action_unique ="",$main_screen_id = "", $user_type = "", $department_name = "", $screen_unique_id_count = "", $action_unique_id_count = "", $action_unique_ids = "",$screen_unique_ids = "")
{

    // dd($screen_unique_ids);
    $total_ui = '';
    ...
    if ($main_screen_id) {
        ...
        foreach ($screen_sections as $section_value) {
            ...
            // Get Section Based Screens
            $user_screens = user_screen_check_up($section_unique_id);
            $user_actions = user_actions_check();
            ...
            //                             <div class="toggle-button__icon"></div>
            //                         </span>';
            $total_ui .= '<button onclick="check_all(\'all' . $section_unique_id . '\',this),take_value(\'screen' . $section_unique_id . '\',this)" type="button" data-id = "all' . $section_unique_id . '" data-check="checked" class="btn btn-sm btn-soft-info btn-rounded waves-effect waves-light" value="unchecked">All</button>';
            $total_ui .= '</th>';
            ...
            foreach ($user_actions as $action_value) {
                ...
                $total_ui .= '<th><button onclick="check_all(\'' . $action_name . $section_unique_id . '\',this),take_value(\'screen' . $section_unique_id . '\',this)" data-check="unchecked" type="button" data-id = "' . $action_name . $section_unique_id . '" class="btn btn-sm btn-soft-info btn-rounded waves-effect waves-light" value="unchecked">' . $action_name . '</button></th>';
            }
            ...
            foreach ($user_screens as $screen_value) {
                ...
                $screen_unique_id = $screen_value->unique_id;
                // dd($screen_unique_id);
                $department_names = $screen_value->department_id;
                // dd($department_name);
                $screen_actions = explode(",", $screen_value->actions);
                ...
                //    dd($screen_unique_id);
                    ...
                    if (empty($screen_perm)) {
                        ...
                        foreach($screen_perm as $screens){
                            $screen_perm = explode(",",$screens->action_unique_id);
                            // dd($screens->action_unique_id);
                            if (count($screen_actions) == count($screen_perm)) {
        ...
        $total_ui .= '<td><span class="toggle-button toggle-button--tuuli"><input class = "checkall' . $section_unique_id . '" onclick="check_all(\'screen' . $screen_unique_id . '\',this),take_value(\'screen' . $screen_unique_id . '\',this)"  name="checkall" id="checkall' . $screen_unique_id . '" value="' . $screen_unique_id . '" type="checkbox" '. $all_checked .'  ><label for="all' . $screen_unique_id . '"></label><div class="toggle-button__icon"><input type="hidden" name="screen_id[]" id="screen_id[]" ></div></span></td>';
// value='.$screen_unique_id.'if($screen_unique_id){ echo "<input type="text" name="screen_id" id="screen_id" value='.$screen_unique_id.' >"}else{ echo "<input type="text" name="screen_id" id="screen_id" >"}
}
                foreach ($user_actions as $action_value) {
...
function get_permissions1($user_type = "",$screen_unique_id = "",$department_name = "",$main_screen = null, $screen_section = null, $screens = null, $screen_actions = null)
{
    // dd($screen_actions);
    // dd($user_type.'and'.$screen_unique_id.'and'.$department_name);
    ...
    if ($user_type) {
        ...
        $result_values = DB::table('user_screen_permission')
                ...
                // ->select('user_type','main_screen_unique_id','screen_unique_id',DB::raw('group_concat(action_unique_id) as action_unique_id'))
                ->select(DB::raw('group_concat(action_unique_id) as action_unique_id'))
                // ->select(DB::raw('group_concat(action_unique_id) as action_unique_id'))
                ->where([['is_delete','=',0],['screen_unique_id','=',$screen_unique_id],['user_type','=',$user_type],['department_name','=',$department_name]])
                    // ->where(['screen_unique_id',$screen_actions])
                    // ->groupBy('action_unique_id')
                ->get();
        // dd($result_values);
...
// ui update end pavilap
// ========================================mythili=============================================================
// Main Screen Function
function roving_squad($unique_id = "")
{
    
    // $vehicle_brands = DB::table('roving_squad')->all();
    // $roving_squads = DB::table('roving_squad')->first()->unique_id;
    //     // $selected_vehicle_brand = VehicleBrand::first()->vehicle_brand_id;

    //     return view('offence_vehicle.edit', compact(['roving_squad_number', 'unique_id'], ['roving_squads']));
...
// offence Section and name


function offence_section_name_id($unique_id = "")
{

    $offence_sections = DB::table('offence_section')->select("offence_name")->where(["is_active" => 1,"is_delete" => 0,"unique_id"=>$unique_id])->orderBy('id', 'ASC')->get();
...
function offence_section_name($unique_id = "")
{

    $offence_section_names = DB::table('offence_section')->select("unique_id", "offence_name")
    ->where(["is_active" => 1,"is_delete" => 0])->groupBy('offence_name')->orderBy('id', 'ASC')->get();


    // $main_screens = $pdo->select($table_details, $where,'','',$order_by);

    if ($offence_section_names) {
        // print_r($offence_section_names);
        return $offence_section_names;
    } else {
        print_r($offence_section_names);
        return 0;
    }
...
function offence_section($unique_id = "")
{
    ...
    $offence_sections = DB::table('offence_section')->select("unique_id","offence_section")->where(["is_active" => 1,"is_delete" => 0])->groupBy('offence_section')->orderBy('id', 'ASC')->get();
...
function offence_name($unique_id = "")
{
