        ...
        <div class="row">
            <div class="col-md-6">
                <div class="breadcrumb">
                    <h1 class="mr-2">Division Creation List</h1>
                    <ul>
                        <li><a href="#">Settings</a></li>
                        <li>Division Creation List</li>
                    </ul>
                </div>
            </div>
            <div class="col-md-6">
                <div class="add_btn text-right">
                    <button class="btn btn-info m-1" onclick="window.location='{{ route('division_creation.create') }}'" type="button"><i class="i-Add"></i> Add New</button></a>
                </div>
            </div>
        </div>
                ...
                <div class="table-responsive">
                    ...
                    <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                        ...
                        <tbody class="table_body">
                            @foreach ($divison_creations as $division)
                                <tr>
                                    <td>{{ ++$i }}</td>
                                    ...
                                    <?php
                                        if ($division->is_active == 1) {
                                                    $division->is_active_value = "Active";
                                                    $color = "#17cd17";
                                        } elseif ($division->is_active == 0) {
                                                    $division->is_active_value = "In Active";
                                                    $color = "tomato";
                                        }
                                    ?>
                                    <td><span class="badge badge-pill badge-danger" style="background-color:{{$color}}">{{ $division->is_active_value }}</span></td>
                                    <td>
                                        <form id="delete-user-form" method="POST" action="{{ route('division_creation.destroy',$division->id) }}">
                                                @csrf
                                                @method('DELETE')
                                                  <a class="btn btn-info" href="{{ route('division_creation.edit',  $division->id) }}"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a>
                                                    <!-- <button type="submit" class="btn btn-xs btn-danger btn-flat show_confirm" data-toggle="tooltip" title='Delete'>Delete</button> -->
                                                    <button type="button" class="show_confirm btn btn-xs btn-danger btn-flat " data-toggle="tooltip"  title='Delete'>
