        ...
        <div class="row">
            <div class="col-md-6">
                <div class="breadcrumb">
                    <h1 class="mr-2"> Consignor/Consignee circle Creation List</h1>
                    <ul>
                        <li><a href="#">Settings</a></li>
                        <li> Consignor/Consignee circle Creation List</li>
                    </ul>
                </div>
            </div>
            <div class="col-md-6">
                <div class="add_btn text-right">
                    <button class="btn btn-info m-1" onclick="window.location='{{ route('consignorcreation.create') }}'" type="button"> <i class="i-Add"></i> Add New</button>
                </div>
            </div>
        </div>
            ...
            <div class="table-responsive">
                ...
                <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                    <thead>
                        <tr>
                            <th>S.NO.</th>
                            <th>Consignor/Consignee Circle Code</th>
                            <th>Consignor/Consignee Circle </th>
                            <th>Description </th>
                            <th>Active Status</th>
                            <th>Action</th>
                        </tr>
                    ...
                    <tbody class="table_body">
                        @foreach ($consignorCreations as $consignorCreation)
                        <tr>
                            <td>{{ ++$i }}</td>
                            <td>{{ $consignorCreation->consignee_code }}</td>
                            <td>{{ $consignorCreation->consignee_circle }}</td>
                            <td>{{ $consignorCreation->description }}</td>
                            <?php
                            if ($consignorCreation->is_active == 1) {
                                $consignorCreation->is_active_value = "Active";
                                $color = "#17cd17";
                            } elseif ($consignorCreation->is_active == 0) {
                                $consignorCreation->is_active_value = "In Active";
                                $color = "tomato";
                            }
                            ?>
                            <td><span class="badge badge-pill badge-danger" style="background-color:{{$color}}">{{ $consignorCreation->is_active_value }}</span></td>
                            <td>
                                <form id="delete-user-form" method="POST" action="{{ route('consignorcreation.destroy',$consignorCreation->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <a class="btn btn-info" href="{{ route('consignorcreation.edit',  $consignorCreation->id) }}"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a>
