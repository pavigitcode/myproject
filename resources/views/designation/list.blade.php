...
<div class="main-content-wrap d-flex flex-column" style="margin-top: 60px;">
    ...
    <div class="main-content">
        <div class="row">
            <div class="col-md-6">
                <div class="breadcrumb">
                    <h1 class="mr-2">Designation Creation List</h1>
                    <ul>
                        <li><a href="#">Settings</a></li>
                        <li>Designation Creation List</li>
                    </ul>
                </div>
            </div>
            <div class="col-md-6">
                <div class="add_btn text-right">
                    <button class="btn btn-info m-1" onclick="window.location='{{ route('designation_creation.create') }}'" type="button"><i class="i-Add"></i> Add New</button></a>
                </div>
            </div>
        </div>
        ...
        <div class="card text-left" style="margin-top: 8px;">
            <div class="card-body">
                {{-- <a href="{{('/prepriview')}}" class="btnprn btn">print</a> --}}
                ...
                <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%" >
                    ...
                    <tbody class="table_body">
                        @foreach ($designation_creations as $designation)
                        <tr>
                            <td>{{ ++$i }}</td>
                            ...
                            if ($designation->is_active == 1) {
                                $designation->is_active_value = "Active";
                                $color = "#17cd17";
                            } elseif ($designation->is_active == 0) {
                                $designation->is_active_value = "In Active";
                                $color = "tomato";
                            }
                            ?>
                            <td><span class="badge badge-pill badge-danger" style="background-color:{{$color}}">{{ $designation->is_active_value }}</span></td>
                            <td>
                              <form method="POST" action="{{ route('designation_creation.destroy', $designation->id) }}" >

                                <a class="btn btn-info" href="{{ route('designation_creation.edit',  $designation->id) }}"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a>
                                @csrf
                                @method('DELETE')
