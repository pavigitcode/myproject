        ...
        <div class="row">
            <div class="col-md-6">
                <div class="breadcrumb">
                    <h1 class="mr-2">Shift Creation List</h1>
                    <ul>
            ...
            <div class="col-md-6">
                <div class="add_btn text-right">
                    <button class="btn btn-info m-1" onclick="window.location='{{ route('shift.create') }}'" type="button"> <i class="i-Add"></i> Add New</button>
                </div>
            </div>
        </div>
                ...
                <table class="display table table-striped table-bordered" id="zero_configuration_table" style="width:100%">
                    ...
                    <tbody class="table_body">
                        ...
                        <tr>
                            ...
                            <td>{{ $id_val }}</td>
                            <?php
                            if ($shift->shift_name == 1) {
                                $shift->shift_name_value = "Day";
                            } elseif ($shift->shift_name == 2) {
                                $shift->shift_name_value = "Night";
                            }
                            ?>
                            <td>{{ $shift->shift_name_value }}</td>
                            <td>{{ $shift->from_time }} to {{ $shift->to_time }}</td>
                            <td>{{ $shift->description }}</td>
                            <?php
                            if ($shift->is_active == 1) {
                                $shift->is_active_value = "Active";
                                $color = "#17cd17";
                            } elseif ($shift->is_active == 0) {
                                $shift->is_active_value = "In Active";
                                $color = "tomato";
                            }
                            ?>
                            <td><span class="badge badge-pill badge-danger" style="background-color:{{$color}}">{{ $shift->is_active_value }}</span></td>
                            <td>
                                
                                <form id="delete-user-form" method="POST" action="{{ route('shift.destroy',$shift->id) }}">
                                    @csrf
                                    @method('DELETE')
                                    <!-- <a class="btn btn-info" href="{{ route('shift.edit',  $shift->id) }}"><i class="nav-icon i-Pen-2 font-weight-bold"></i></a> -->
